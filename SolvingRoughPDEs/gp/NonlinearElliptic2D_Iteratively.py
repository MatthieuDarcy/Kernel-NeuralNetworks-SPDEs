...
"""
This file consider both sin and cos eigen functions and iteratively solve the PDEs with high frequency
"""
...
class NonlinearElliptic2D(object):
    ...
    def set_data(self, samples, M, M_Omega, xi):
    ...
    def u(self, x, y):
        func = lambda xi_i, xi_j, i, j: i ** self.alpha * xi_i \
                                         ...
                                         * jnp.sin(j * jnp.pi * y)
        vals = func(self.xi[:self.mm, 0], self.xi[:self.mm, 1], self.u_func_indices[:, 0], self.u_func_indices[:, 1])
        return jnp.sum(vals)
    ...
    def train(self, cfg, u0_weights = jnp.array([]), u0_hs = jnp.array([]), u0_z = jnp.array([])):
        ...
        if u0_weights.size != 0 and u0_hs.size !=0 and u0_z.size !=0:
            uk_weights = u0_weights
            uk_hs = u0_hs
            uk_z = u0_z
