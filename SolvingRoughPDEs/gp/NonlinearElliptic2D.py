...
class NonlinearElliptic2D(object):
    ...
    def __init__(self, kernel, domain, alpha, beta, m, N, s, gamma, deg):
        ...
        self.scalers = jnp.array(jnp.arange(0, N) + 1)
        sXX, sYY = jnp.meshgrid(self.scalers, self.scalers, indexing='ij')
    ...
    def sample_f(self, m):
        self.fxi = np.concatenate((np.random.uniform(0, 1, (m, 1)), np.random.uniform(0, 1, (m, 1))), axis=1)
    ...
    def u(self, x, y):
        ...
        # func = lambda i, xi_i, j, xi_j: i ** self.alpha * xi_i \
        ...
        # vals = func(scalers, self.fxi[0, :], scalers, self.fxi[1, :])
        # return jnp.sum(vals)
