...
parser.add_argument('--n_meas_max', type=int, help='Maximum number of measurements (in log 2 scale)')
...
parser.add_argument('--n_meas_min', type=int, default=0, help='Minimum number of measurements')
...
parser.add_argument('--n_order', type=int, default=35, help='Order of the quadrature rule')
# add an argument for the number of coefficients
parser.add_argument('--n_coef', type=int, default=5000, help='Number of coefficients')
...
for i in n_meas_list:
    ...
    N_test_functions = diracs.shape[0]

    print("Number of test functions: ", N_test_functions)


    # Compute the RHS of the linear system
    f_meas = evaluate_function(diracs, coef_f, L)
    ...
    theta_12 = vmap_kernel_laplacian_y(boundary, diracs, length_scale)
    theta_22 = compute_K_double_laplacian_pairwise(diracs, diracs, length_scale)
...
log_n_meas = jnp.log(2**n_meas_list)
...
ax[0,0].plot(2**n_meas_list, error_list, label = r"$r$ = {}, C = {}".format(r, C))
ax[0,0].scatter(2**n_meas_list, error_list)
...
ax[0,1].plot(2**n_meas_list, relative_error_list,  label = r"$r$ = {}, C = {}".format(r, C))
ax[0,1].scatter(2**n_meas_list, relative_error_list)
...
ax[1,0].plot(2**n_meas_list, error_list_h, label = r"$r$ = {}, C = {}".format(r_h, C))
ax[1,0].scatter(2**n_meas_list, error_list_h)
...
ax[1,1].plot(2**n_meas_list, relative_error_list_h, label = r"$r$ = {}, C = {}".format(r_h, C_h))
ax[1,1].scatter(2**n_meas_list, relative_error_list_h)
...
for i, pred in enumerate(pred_list):
    ax[0].plot(x, pred, label = "{}".format(2**n_meas_list[i]))
    ax[0].set_xlabel("x")
    ax[0].set_ylabel(r"$u^*(x)$")
    ax[0].set_title("Numerical solution")

    ax[1].plot(x, pred - u_values, label = "{}".format(2**n_meas_list[i]))
    ax[1].set_xlabel("x")
...
def update(i):
        ...
        fig.suptitle(f"Level {i} ({N_test_functions} measurements)")

        ax[0].clear()
        ax[0].scatter(diracs, jnp.ones(diracs.shape[0]), s = 10, c = "red")
        ax[0].set_xlim(lower- 0.1, upper+0.1)
        ax[0].set_xlabel("x")
        ax[0].set_title("Measurements", size = 10)

        ax[1].clear()
        ax[1].plot(x, pred_list[i], label = 'pred')
        ax[1].plot(x, u_values, label = 'true', color = "red")
        ax[1].set_xlabel("x")
        ax[1].set_title("Solution", size = 10)
        ax[1].legend()

        ax[2].clear()
        ax[2].plot(x, pred_list[i] - u_values)
        ax[2].set_xlabel("x")
        ax[2].set_ylabel(r"$u^\dagger(x) - u^*(x)$")
        ax[2].set_title("Pointwise error", size = 10)


animation = FuncAnimation(fig, update, frames=len(n_meas_list), interval=2000, repeat=False)
animation.save(save_folder+"results.mp4", writer='ffmpeg', fps=1, dpi = 150)
